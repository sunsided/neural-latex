...
While in \algo{gradient descent} the step width along the gradient of the error function $\nabla J(\thetavec)$ is given by a pre-determined parameter $\lambda$ (think "learning rate"), the optimal solution would be to select the step width $\lambda$ in such a way that it minimizes the function along the direction of the gradient $\vec{d} = -\nabla J(\thetavec)$, such that
%
\begin{align}

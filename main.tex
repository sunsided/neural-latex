...
Since the first derivative of a function describes the slope of that function at the point of evaluation, it can be used to determine the direction of the nearest local maximum (or minimum).
\Cref{fig:sselike_cost} \vpageref{fig:sselike_cost} visualizes this using the function $f(a,b)=a^2-ab$.
...
\begin{figure}[H]
	...
	\caption[Surface plot of $f(a,b)=a^2-ab$.]{Surface plot of $f(a,b)=a^2-ab$. Shows the cost with respect to the parameters $a,b$. Two local minima exist, one on either side of the valley.}
	...
	\caption[Quiver plot of $f(a,b)=a^2-ab$.]{Gradient vector field for $f(a,b)=a^2-ab$. Length and direction of the vectors represent magnitude and sign of the partial derivative, which can be interpreted as the "uphill" direction.}
	\label{fig:sselike_cost_quiver}
	\end{subfigure}
	
	\caption{Visualization of $f(a,b)=a^2-ab$ and its partial derivatives.}
...
\Cref{lst:matlab-2nd-derivative} determines the Laplacian (i.e. the second derivative) of the squared error function as
...
can be used instead of the SSE. \todo{reference to Andrew Ng}

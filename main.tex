...
The sum rule of differentiation gives that the derivative of a sum is the sum of the derivatives, such that \cref{eq:nablaJ} becomes
%
\begin{align}
...
By application of the chain rule, we find that
%
\begin{align}
\frac{\partial}{\partial \theta} \frac{1}{2m} \left( h_\theta(x^{(i)}) - y^{(i)}\right)^2 &= 
%
\frac{1}{\cancel{2}m} \underbrace{\cancel{2} \left( h_\theta(x^{(i)}) - y^{(i)}\right)}_{\text{outer derivative}}
%
\enskip \cdot \enskip
\underbrace{

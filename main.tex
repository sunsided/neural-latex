...
The cost function $J(\theta)$ is not a function of the training pairs $\{x, y\}$ -- as it is calculated over the complete training set --, but rather a function of the network's parameter $\theta$ themselves.
In order to determine in which way changing any of these parameters will change cost, we will inspect the partial derivative of $J(\theta)$ with respect to each individual component $\theta_j \in \theta$.

Since the first derivative of a function describes the slope of that function at the point of evaluation, it can be used to determine the direction of the nearest local maximum (or minimum), which is visualized by 
\cref{fig:sselike_cost} \vpageref{fig:sselike_cost} using the function $f(a,b)=3a^2+b^2$.
Consequently, the set of all partial derivatives of $J(\theta)$ with respect to $\theta$ describes the direction in parameter space in which the nearest local maximum (or, by sign flip, minimum) of the cost function can be found.
...
The \textit{Laplacian} (second derivative) of the squared-error function can be obtained by differentiating \cref{eq:sse1stDerivative}, such that

\begin{align}
\nabla^2 J(\theta) = \frac{\partial^2}{\partial \theta^2} J(\theta) &=
...
In the first part, the sum rule is applied again, giving $\sfrac{\partial}{\partial \theta}
\left( h_\theta(x^{(i)}) - y^{(i)}\right) = \sfrac{\partial}{\partial \theta}\,
h_\theta(x^{(i)}) - \sfrac{\partial}{\partial \theta}\, y^{(i)}$. Since $y^{(i)}$ is independent of $\theta$, it follows that $\sfrac{\partial}{\partial \theta}\, y^{(i)} = 0$, such that
...
The Matlab script in \Cref{lst:matlab-2nd-derivative} consequently prints out the same result, apart from a factored-out sign.

...
In order to determine the neural connection weights $\thetavec$, a multitude of local and global optimization algorithms can be used.
One of the most widely used (or at least discussed) algorithms \todo{yeah ... where?} is the \algo{gradient descent}, described in \cref{sec:gradientdescent}.
...
By choosing the next evaluation point using a line search (\cref{sec:linesearch}), the gradient descent becomes the \algo{steepest descent} algorithm.

\subsection{Line search minimization}
\label{sec:linesearch}

A \algo{line search} is the one-dimensional minimization of a function $f(\vec{x})$ given a search direction $\vec{d}$:
%
\begin{align}
\lambda_{min} &= \argmin_\lambda f(\vec{x} + \lambda \vec{d}) \\
\vec{x}_{min} &= \vec{x} + \lambda \vec{d}
\end{align}
%
where $\vec{x}_{min}$ is the minimum of $f(\vec{x})$ along the direction $\vec{d}$, starting from point $\vec{x}$.

While in \algo{gradient descent} the step width along the gradient of the error function $\nabla J(\thetavec)$ is given by a pre-determined parameter $\lambda$ (think "learning rate"), a more optimal solution would be to select the step width $\lambda$ in such a way that it minimizes the function along the direction of the gradient $\vec{d} = -\nabla J(\thetavec)$, such that
%
\begin{align}
\lambda &= \argmin_\lambda J(\thetavec - \lambda \, \nabla J(\thetavec)) \\
\hat\thetavec &= \thetavec - \lambda \, \nabla J(\thetavec)
\end{align}
%
where $\hat\thetavec$ would be the next point of gradient evaluation.
Such an algorithm is called the \algo{steepest descent}.

\subsubsection{Hessian approximation from gradient}

Even though gradient information is suggested not to be used actively in line searches due to the computational overhead (\cite[ch.~7.6]{Bishop:1995:NNP:525960}), the optimal step length $\lambda$ for a quadratic error function can be calculated given the Hessian $\mat{H}$ as
%
\begin{align}
\lambda &= -\frac{\vec{d}\transp \nabla J(\thetavec)}{\vec{d}\transp \mat{H} \vec{d}}
\label{eq:cgquadraticlambda}
\end{align}
%
where $\vec{d} = -\nabla J(\thetavec)$ at point $\thetavec$ (\cite[ch.~7.7.2]{Bishop:1995:NNP:525960}).

In this case we can make use of the fact that
%
\begin{align}
\vec{d}\transp \mat{H} &\equiv \vec{d}\transp \nabla \left(\nabla J(\thetavec) \right)
\end{align}
%
and 
%
\begin{align}
\left( \vec{d}\transp \mat{H} \right) \vec{d}
   = \vec{d}\transp \left( \mat{H} \vec{d} \right)
\end{align}
%
...
\begin{align}
\nabla_i \left(\nabla J(\thetavec) \right)
  &= \frac{\partial}{\partial \thetavec_i} \left(\nabla J(\thetavec) \right) \\
  &= \lim_{\epsilon \to 0+} \frac{\nabla J(\thetavec + \epsilon \vec{1}_i) - \nabla J(\thetavec)}{\epsilon}
\end{align}
%
with $\vec{1}_i$ being the unit vector along dimension $i$, we obtain the first-order finite difference approximation
%
\begin{align}
\nabla \left(\nabla J(\thetavec) \right) \, \vec{d}
  &\approx \frac{\nabla J(\thetavec + \epsilon \vec{d}) - \nabla J(\thetavec)}{\epsilon}
\end{align}
%
for small $\epsilon$, leaving residuals of $\order{\epsilon}$ (\cite{Pearlmutter94fastexact,Bishop:1995:NNP:525960}).

Because this approximation will be subject to machine precision roundoffs, the central difference should be used instead, which gives
%
\begin{align}
\nabla \left(\nabla J(\thetavec) \right) \, \vec{d}
  &\approx \frac{\nabla J(\thetavec + \epsilon \vec{d}) - \nabla J(\thetavec - \epsilon \vec{d})}{2\epsilon}
\end{align}
%
with residuals of $\order{\epsilon^2}$ (\cite{Bishop:1995:NNP:525960}).

\subsection{Conjugate gradient descent algorithm}
\label{sec:conjgradientdescent}

Uses a line search, as described in \cref{sec:linesearch}.
...
A widely used and relatively simple differentiable error metric is the \algo{sum of squared errors} (SSE) that sums the (squared) errors over all training examples $\{x^{(i)}, y^{(i)}\}$:
%
\begin{align}
...
\todo[inline]{delete that chapter}
...
\subsection{Logistic cost}

For binary classification problems, i.e. when $y_i \in \{0, 1\}$, and when using sigmoidal activation functions, the logistic cost function
%
\begin{align}
...
\end{align}
%
is suggested to be used instead of the squared error (\cite{Bishop:1995:NNP:525960,NgCourseraML2015}).
